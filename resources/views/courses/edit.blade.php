...
<div class="max-w-3xl mx-auto">
    <div class="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg border border-serene-100 p-8">
        <h1 class="text-2xl font-bold text-gray-800 mb-8">Edit Kursus</h1>
        ...
        <form action="{{ route('courses.update', $course) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="5" required
                          class="w-full px-4 py-3 border border-serene-200 rounded-xl focus:border-serene-500 focus:ring-4 focus:ring-serene-100 transition text-base resize-none">{{ old('description', $course->description) }}</textarea>
                @error('description')
                    <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
